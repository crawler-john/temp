...
int set_encryption(const char *recvbuffer, char *sendbuffer)
{
	...
	strncpy(timestamp, &recvbuffer[33+passwordlth+4], 14);
	if(passwordlth>0)
...
//		{
		...
		{
			if(recvbuffer[33+passwordlth]=='A')	//不用设置时间
			{
				sprintf(sql,"SELECT * FROM key where item=1");
				sqlite3_get_table( db , sql , &azResult , &nrow , &ncolumn , &zErrMsg );
				memset(sql,'\0',1024);
			...
			{
				tmout=msg_get_int(&recvbuffer[33+passwordlth], 4);
				memset(sql,'\0',1024);
				sprintf(sql,"REPLACE INTO key (item,key,operator,cmd,set_flag,read_flag,set_time_flag,timeout) VALUES(1,'%s',1,1,1,0,1,'%d')",password,tmout);
				sqlite3_exec(db, sql, 0, 0, &zErrMsg);
			}
		}
		else	//只设置时间
		{
			...
			memset(sql,'\0',1024);
			if(nrow==0)
				sprintf(sql,"INSERT INTO key (item,operator,cmd,set_flag,read_flag,set_time_flag,timeout) VALUES(1,1,1,0,0,1,'%d')",tmout);
			else
				sprintf(sql,"UPDATE key SET timeout='%d',operator=1,cmd=1,set_time_flag=1 WHERE item=1",tmout);
			sqlite3_exec(db, sql, 0, 0, &zErrMsg);
			perror(zErrMsg);
			sqlite3_close(db);
