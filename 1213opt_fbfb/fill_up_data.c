 ...
 * 8.替换datetime.c文件
 * 9.main.c文件中，float ecu.life_energy系统历史总发电量改为全局变量，方便fill_up_data.c文件中调用
...
extern int zbmodem;			//PLC的文件描述符
extern int caltype;			//计算方式，NA版和非NA版的区别
//extern int ecu.total;
extern int zoneflag;			//修改时区的标志，改动为1， 没改动为0
//extern float ecu.life_energy;			//系统历史总发电量
...
//	time_linux=time(&tm);
	...
	sendbuff[2] = 0x02;			//FROM
	sendbuff[3] = 0x03;			//TO
	sendbuff[4] = 0x11;			//LENGTH
	sendbuff[5] = 0x00;			//
	sendbuff[6] = 0x00;			//
	sendbuff[7] = 0x0D;			//CMD
	sendbuff[8] = time_linux/16777216;
	sendbuff[9] = (time_linux%16777216)/65536;
	sendbuff[10] = ((time_linux%16777216)%65536)/256;
	sendbuff[11] = ((time_linux%16777216)%65536)%256;
	sendbuff[12] = 0x00;
	sendbuff[13] = 0x00;
	sendbuff[14] = 0x00;
	sendbuff[15] = 0x00;
	...
	zb_broadcast_cmd(sendbuff, 26);
	printhexmsg("sendbuff",sendbuff,26);
...
int clear_single_time(int time_linux,char* inverterid)		//清除单个逆变器的单个时间
{
	...
	sqlite3_open("/tmp/replacement.db", &db);
	sprintf(sql, "DELETE FROM save_replacement_time WHERE inverterid = '%s' AND time = %d", inverterid ,time_linux);
	sqlite3_exec_3times(db, sql);printf("%s\n,",sql);
	sqlite3_close(db);
	return 1;
}
...
int query_lost_data(struct inverter_info_t *inverter , int time, char *data, char* receivetime)		//向逆变器读取丢失的数据
{
	unsigned char sendbuff[512] = {'\0'};
	unsigned char readbuff[512] = {'\0'};
	unsigned short check = 0x00;
//	char receivetime[20]={'\0'};
	...
	sendbuff[7] = 0x0F;			//CMD
	sendbuff[8] = 0x00;
	sendbuff[9] = time/16777216;
	sendbuff[10] = (time%16777216)/65536;
	sendbuff[11] = ((time%16777216)%65536)/256;
	sendbuff[12] = ((time%16777216)%65536)%256;
	sendbuff[13] = 0x00;
	sendbuff[14] = 0x00;
	sendbuff[15] = 0x00;
	sendbuff[16] = 0x00;
	...
	zb_send_cmd(inverter, sendbuff, 26);

	printhexmsg("sendbuff",sendbuff,26);
	ret = zb_get_reply(readbuff,inverter);

	if((85==ret)&&(0xFB == readbuff[0])&&(0xFB == readbuff[1])&&(0x03 == readbuff[2])&&
						(0x02 == readbuff[3])&&(0x4C == readbuff[4])&&
						(0x00 == readbuff[5])&&(0x00 == readbuff[6])&&
						(0x0F == readbuff[7])&&
						(0xFE == readbuff[83])&&(0xFE == readbuff[84]))
	{
...
//		tm = readbuff[30]*16777216+readbuff[31]*65536+readbuff[32]*256+readbuff[33];
		...
		if(0x00==readbuff[9])
		{
			for(i=0, j=21; i<58; i++, j++)
			{
				data[i] = readbuff[j];
			}
			clear_single_time(time,inverter->inverterid);
			return ret;
		}
		else
		{
			if(0x01==readbuff[9])				//没查到该时间点
			{printf("no tm\n");
				//clear_single_time(time,inverter->inverterid);
				return -1;
			}
			else if(0x10==readbuff[9])			//时间点小于可查时间
			{
					temp_time=readbuff[17]*16777216+readbuff[18]*65536+readbuff[19]*256+readbuff[20];
					printdecmsg("temp_time",temp_time);
					clear_single_less_than_time(temp_time,inverter->inverterid);
					return 2;
			}
		}
	}
...
//	if(curgeneration>0)
//	{
//		ecu.life_energy = ecu.life_energy + curgeneration;
...
int resolve_lost_data_single_opt(char *inverter_data,char *inverterid, char *receivetime,int last_data_mark)		//解析补发的数据
{

	char sql[1024]={'\0'};
	unsigned char temp[2] = {'\0'};
	int i, seconds;
	char data_EMA_o[256]= {'\0'};
	char data_EMA_a[256]= {'\0'};
	char data_EMA_b[256]= {'\0'};
	char *buff;
	char status_ema[64] = {'\0'};
	...
	/********out路数据***********/
	float opt_output_voltage;
	float opt_output_power;
	float opt_output_current;
	/********A路数据***********/
	float opt_input_voltage_pv1;			//直流电压
	float opt_input_current_pv1;			//直流电流
	float opt_input_power_pv1;			//输出功率
	float np;			//电网频率
	int nv;			//电网电压
	int opt_temperature;			//机内温度

//	int flagyc500;			//1，为YC500

	/********B路数据***********/
	float opt_input_voltage_pv2;			//直流电压
	float opt_input_current_pv2;			//直流电流
	float opt_input_power_pv2;			//输出功率
	//float npb;			//电网频率
	//int nvb;			//电网电压
	//int itb;			//机内温度
	//float npb;			//电网频率
	...
	opt_output_voltage = (inverter_data[0]*256+inverter_data[1])/10.0;
	opt_input_voltage_pv1 = (inverter_data[2]*256+inverter_data[3])/10.0;
	opt_input_voltage_pv2 = (inverter_data[4]*256+inverter_data[5])/10.0;
	opt_output_power = (inverter_data[6]*256+inverter_data[7])/10.0;
	opt_input_power_pv1 = (inverter_data[8]*256+inverter_data[9])/10.0;
	opt_input_power_pv2 =(inverter_data[10]*256+inverter_data[11])/10.0;
	opt_output_current = (inverter_data[12]*256+inverter_data[13])/10.0;
	opt_input_current_pv1=(inverter_data[14]*256+inverter_data[15])/1.0;
	opt_input_current_pv2=(inverter_data[16]*256+inverter_data[17])/1.0;
	opt_temperature =(inverter_data[18]*256+inverter_data[19])/1.0;
...
//		opb = (int)(dvb*dib/100.0);
	...
	//check_data_opt
	if(opt_output_voltage>999)
		opt_output_voltage=999;
	if(opt_input_voltage_pv1>999)
		opt_input_voltage_pv1=999;
	if(opt_input_voltage_pv2>999)
		opt_input_voltage_pv2=999;
	if(opt_output_power>999)
		opt_output_power=999;
	if(opt_input_power_pv1>999)
		opt_input_power_pv1=999;
	if(opt_input_power_pv2>999)
		opt_input_power_pv2=999;
	if(opt_output_current>999)
		opt_output_current=999;
	if(opt_input_current_pv1>999)
		opt_input_current_pv1=999;
	if(opt_input_current_pv2>999)
		opt_input_current_pv2=999;
	if(opt_temperature>899)
		opt_temperature=899;
	...
	//	save_status_single(inverterid, status_ema, receivetime);		//优化器无保护参数
	}

	sprintf(data_EMA_o,"%s%06d%06d%06d0000000000%03d",inverterid,(int)(opt_output_voltage*1000.0),(int)(opt_output_current*1000.0),(int)(opt_output_power*1000.0),opt_temperature+100);
	sprintf(data_EMA_a, "1%06d%06d%06d%010d",(int)(opt_input_voltage_pv1*1000.0), (int)(opt_input_current_pv1*1000.0), (int)(opt_input_power_pv1*1000), (int)(curgeneration*1000000));
	print2msg("data_EMA_a",data_EMA_a);
	sprintf(data_EMA_b, "2%06d%06d%06d%010d",(int)(opt_input_voltage_pv2*1000.0), (int)(opt_input_current_pv2*1000.0), (int)(opt_input_power_pv2*1000),(int)(curgenerationb*1000000));
	print2msg("data_EMA_b",data_EMA_b);
	strcat(data_EMA_o,data_EMA_a);
	strcat(data_EMA_o,data_EMA_b);
	print2msg("data_EMA_o",data_EMA_o);

//	sprintf(data_EMA_a, "%s1%05d%03d%05d%05d%03d%03d%010d", inverterid, (int)(dv), (int)(di), op*100, (int)(np*10), it+100, nv, (int)(curgeneration*1000000));
//	print2msg("data_EMA_a",data_EMA_a);
...
//	print2msg("data_EMA_b",data_EMA_b);
	...
	save_single_lost_data(data_EMA_o,receivetime);
	//save_single_lost_data(data_EMA_b,receivetime);

	printf("%d\n",__LINE__);
	if((curgeneration>0)||(curgenerationb>0))
	{
		ecu.life_energy = ecu.life_energy + curgeneration + curgenerationb;
		update_lifetime_energy(ecu.life_energy);
		print2msg("receivetime",receivetime);
		printmsg("debug1");
		update_daily_energy((float)(curgeneration + curgenerationb),receivetime);
		printmsg("debug2");
		update_monthly_energy((float)(curgeneration + curgenerationb),receivetime);
		printmsg("debug3");
		update_yearly_energy((float)(curgeneration + curgenerationb),receivetime);
		printmsg("debug4");
	}

	if((opt_input_power_pv1>0)||(opt_input_power_pv2>0))
	{printf("%d\n",__LINE__);
		save_system_power_single((float)(opt_input_power_pv1+opt_input_power_pv2),receivetime);
		printmsg("debug5");
	}printf("%d\n",__LINE__);
}
...
int reslove_save_all_lost_data_opt(char *inverter_data,char *inverterid, char *receivetime)	//解析并保存单个时间所有逆变器丢失的数据到数据库
{
	sqlite3 *db;
	char *zErrMsg=0;
	int nrow=0,ncolumn=0;
...
//		opb = (int)(dvb*dib/100.0);
	...
	sprintf(data_EMA_o, "%s%06d%06d%06d0000000000%03d", inverterid, (int)(opt_output_voltage*1000.0), (int)(opt_output_current*1000.0), opt_output_power*1000,opt_temperature+100);
	print2msg("data_EMA_o",data_EMA_o);
	sprintf(data_EMA_a, "1%06d%06d%06d0000000000", (int)(opt_input_voltage_pv1*1000.0), (int)(opt_input_current_pv1*1000.0), (int)(opt_input_power_pv1*1000));
	print2msg("data_EMA_a",data_EMA_a);
	sprintf(data_EMA_b, "2%06d%06d%06d0000000000END", (int)(opt_input_voltage_pv2*1000.0), (int)(opt_input_current_pv2*1000.0), (int)(opt_input_power_pv2*1000));
	print2msg("data_EMA_b",data_EMA_b);

	strcat(lost_data_info.sendbuff, data_EMA_o);
	strcat(lost_data_info.sendbuff, data_EMA_a);
	strcat(lost_data_info.sendbuff, data_EMA_b);

	print2msg("lost_data_info.sendbuff",lost_data_info.sendbuff);
	lost_data_info.syspower=lost_data_info.syspower+opt_input_power_pv1+opt_input_power_pv2;
	lost_data_info.sysoutpower=lost_data_info.sysoutpower+opt_output_power;
...
int protocol_all_lost_data(char *receivetime, int all_time_linux)	//拼接单个时间所有逆变器丢失的数据
{
	...
	if(lost_data_info.inverter_count>0)				//当至少有一台逆变器获取到数据时候，才进行头拼接并发送到record表,并且清除lost_all_time表中这个时间点
	{
		...
		sprintf(lost_data_info.sendbuff_all, "APS160000000010001%s%010d%010d00000000000000000000%s%03d000000END", ecu.id, (int)(lost_data_info.syspower*1000.0), (int)(lost_data_info.sysoutpower*1000.0), receivetime, lost_data_info.inverter_count);
...
int fill_up_all_data(struct inverter_info_t *firstinverter,int rest_time)		//补单个时间所有逆变器丢失的数据
{
	...
	while(1)
	{
		...
		printdecmsg("ecu.total",ecu.total);
		...
		//	if((rest_time-time(NULL)<25)||(fill_up_times>4))
			...
			if(1 == curinverter->fill_up_data_flag)				//判断此台逆变器有补数据功能才进行补数据操作
			{
				...
				if(85 == flag)
					reslove_save_all_lost_data_opt(data,curinverter->inverterid, receivetime);
			}

			if(i==ecu.total-1)			//当全部逆变器都轮询一次并且至少有一台逆变器获取到数据才进入拼接
			{
				clear_all_time(all_time_linux);
		...
		for(i=lost_data_info.num; (i<ecu.total); i++, lost_data_info.num++)
		{
			...
			while((time_linux=(get_single_time(curinverter->inverterid)))>0)
			{
				...
				if(-1==result)				//-1时直接跳下一台逆变器
					break;
				if(85==result)				//1时解析获取到的数据 YC500
					resolve_lost_data_single_opt(data,curinverter->inverterid, receivetime,0);
