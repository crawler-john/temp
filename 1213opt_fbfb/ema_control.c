...
int save_gfdi_changed_result(inverter_info *firstinverter)
{
	...
	for(i=0; (i<MAXINVERTERCOUNT)&&(12==strlen(inverter->inverterid)); i++, inverter++)
	{
		if(1 == inverter->gfdi_changed_flag)
		{
			...
			strcat(gfdi_changed_result, inverter->inverterid);
...
{printmsg("22");
	...
	for(i=0; (i<MAXINVERTERCOUNT)&&(12==strlen(inverter->inverterid)); i++, inverter++)
	{
		if(1 == inverter->turn_on_off_changed_flag)
		{
			...
			strcat(turn_on_off_changed_result, inverter->inverterid);
...
int save_protect_result(inverter_info *firstinverter)
{
	...
	//	if((-1 != inverter->protect_voltage_min) || (-1 != inverter->protect_voltage_max))
		{
			memset(inverter_result, '\0', sizeof(inverter_result));
			sprintf(inverter_result, "%s%03d%03d%03d%03d%05d", inverter->inverterid, inverter->protect_voltage_min, inverter->protect_voltage_max, (int)(inverter->protect_frequency_min*10), (int)(inverter->protect_frequency_max*10), inverter->recovery_time);
