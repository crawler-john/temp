...
sqlite3 *init_tmpdb(inverter_info *firstinverter)
{
	...
	if(0 == nrow){
		for(i=0; (i<MAXINVERTERCOUNT)&&(12==strlen(curinverter->inverterid)); i++, curinverter++){
			memset(sql, '\0', sizeof(sql));
			sprintf(sql, "INSERT INTO powerinfo VALUES ('%s', 0, 0, 0, 0, 0, 0, 0, 0, '20130101000000');", curinverter->inverterid);
			sqlite3_exec(tmpdb, sql, 0, 0, &zErrMsg);
		}
	...
	for(i=0; (i<MAXINVERTERCOUNT)&&(12==strlen(curinverter->inverterid)); i++, curinverter++){
		memset(sql, '\0', sizeof(sql));
		sprintf(sql, "SELECT preaccgen,preaccgenb,preaccgenc,preaccgend,pre_output_energy,pre_output_energyb,pre_output_energyc,preacctime,last_report_time FROM powerinfo WHERE id='%s';", curinverter->inverterid);
		sqlite3_get_table( tmpdb , sql , &azResult , &nrow , &ncolumn , &zErrMsg );
		if(1 == nrow){
...
#if 0
int get_protect_data_from_db(char *presetdata)			//读取用户设置的配置参数值
{
	char sql[100] = {'\0'};
	char *zErrMsg=0;
...
{
    char sql[1024]={'\0'};
    char db_buff[MAXINVERTERCOUNT*200+100]={'\0'};
    char *zErrMsg=0;
...
int get_id_from_db(inverter_info *firstinverter)	//获取逆变器的ID号
{
	...
	sqlite3_get_table( db , sql , &azResult , &nrow , &ncolumn , &zErrMsg );
	print2msg("Get id from database", zErrMsg);

	for(i=1;i<=nrow;i++)
	{
		strcpy(inverter->inverterid, azResult[i*ncolumn]);
	...
	for(i=1; i<=nrow; i++, inverter++)
		printdecmsg(inverter->inverterid, inverter->shortaddr);
	printmsg("--------------");
...
//	int recovery_time;					//并网恢复时间/开机时间
	...
	sprintf(sql,"UPDATE id SET actual_min_protect_voltage=%d,actual_max_protect_voltage=%d,actual_min_protect_frequency=%.1f,actual_max_protect_frequency=%.1f,actual_boot_time=%d WHERE id='%s' ",inverter->protect_voltage_min,inverter->protect_voltage_max,inverter->protect_frequency_min,inverter->protect_frequency_max,inverter->recovery_time,inverter->inverterid);
...
void update_inverter_model_version(inverter_info *inverter)					//更新逆变器的机型码和软件版本号
{
	...
	sprintf(sql,"UPDATE id SET model=%d,software_version=%d WHERE id='%s' ",inverter->model,inverter->version,inverter->inverterid);
//	sprintf(sql,"UPDATE id SET model=%d WHERE id='%s' ",inverter->model,inverter->inverterid);
	sqlite3_exec( db , sql , 0 , 0 , &zErrMsg );
...
void update_inverter_bind_flag(inverter_info *inverter)					//更新逆变器的绑定标志位
{
	...
	sprintf(sql,"UPDATE id SET bind_zigbee_flag=%d WHERE id='%s' ",1,inverter->inverterid);
	sqlite3_exec( db , sql , 0 , 0 , &zErrMsg );
...
void update_zigbee_version(inverter_info *inverter)					//更新zigbee的版本号
{
	...
	sprintf(sql,"UPDATE id SET turned_off_rpt_flag=%d WHERE id='%s' ",inverter->zigbee_version,inverter->inverterid);
	sqlite3_exec( db , sql , 0 , 0 , &zErrMsg );
...
void update_today_energy(float currentpower)			//更新系统当天的发电量，每次把当前一轮的发电量传入，把当天的发电量读出，相加后再存入数据库
{
	...
	getdate(date);
	strcpy(sql,"SELECT * FROM today_energy");
	sqlite3_get_table( db , sql , &azResult , &nrow , &ncolumn , &zErrMsg );

	if(nrow==0)
	{
		memset(sql,'\0',100);
		sprintf(sql,"INSERT INTO \"today_energy\" VALUES('%s',%f);",date,currentpower);
		sqlite3_exec( db , sql , 0 , 0 , &zErrMsg );
		print2msg("zErrMsg",zErrMsg);
	...
	else if(!strcmp(azResult[2],date))
	{
		...
		memset(sql,'\0',100);
		sprintf(sql,"UPDATE today_energy SET today_energy=%f WHERE date='%s' ",todaypower,date);
		sqlite3_exec( db , sql , 0 , 0 , &zErrMsg );
		print2msg("zErrMsg",zErrMsg);
	}
	else
	{
		memset(sql,'\0',100);
		sprintf(sql,"DELETE FROM today_energy WHERE date = '%s'",azResult[2]);//
		sqlite3_exec( db , sql , 0 , 0 , &zErrMsg );
		memset(sql,'\0',100);
		sprintf(sql,"INSERT INTO \"today_energy\" VALUES('%s',%f);",date,currentpower);
		sqlite3_exec( db , sql , 0 , 0 , &zErrMsg );
...
int save_optimizer_data(struct inverter_info_t *inverter, char *date_time)
{
	sqlite3 *db;
	char *zErrMsg=0;
	char sql[1024];
	int i;
	char date_time_optimizer_day[9]={'\0'};
	char data_time_format[20]={'\0'};


	sqlite3_open("/home/optimizer.db", &db);

	strncpy(date_time_optimizer_day,date_time,8);

	memset(data_time_format, '\0', 20);

	data_time_format[0] = date_time[0];
	data_time_format[1] = date_time[1];
	data_time_format[2] = date_time[2];
	data_time_format[3] = date_time[3];
	data_time_format[4] = '-';
	data_time_format[5] = date_time[4];
	data_time_format[6] = date_time[5];
	data_time_format[7] = '-';
	data_time_format[8] = date_time[6];
	data_time_format[9] = date_time[7];
	data_time_format[10] = ' ';
	data_time_format[11] = date_time[8];
	data_time_format[12] = date_time[9];
	data_time_format[13] = ':';
	data_time_format[14] = date_time[10];
	data_time_format[15] = date_time[11];
	data_time_format[16] = ':';
	data_time_format[17] = date_time[12];
	data_time_format[18] = date_time[13];



	sprintf(sql, "CREATE TABLE IF NOT EXISTS date%s (id VARCHAR(15), date_time VARCHAR(20), output_voltage REAL, output_current REAL, output_power REAL, input_voltage_pv1 REAL, input_current_pv1 REAL, input_power_pv1 REAL, input_voltage_pv2 REAL, input_current_pv2 REAL, input_power_pv2 REAL, temperature INTEGER)",date_time_optimizer_day);
	...
	if(SQLITE_OK == sqlite3_exec( db , sql , 0 , 0 , &zErrMsg ))
	{
		printmsg(zErrMsg);
		memset(sql, '\0', sizeof(sql));
		sprintf(sql, "INSERT INTO date%s (id, date_time, output_voltage, output_current, output_power, input_voltage_pv1, input_current_pv1, input_power_pv1, input_voltage_pv2, input_current_pv2, input_power_pv2, temperature) VALUES('%s', '%s', %.1f, %.1f, %.1f, %.1f, %.1f, %.1f, %.1f, %.1f, %.1f, %d)", date_time_optimizer_day, inverter->inverterid, data_time_format, inverter->output_voltage,inverter->output_current, inverter->output_power, inverter->input_voltage_pv1, inverter->input_current_pv1, inverter->input_power_pv1, inverter->input_voltage_pv2, inverter->input_current_pv2, inverter->input_power_pv2, inverter->temperature);
		for(i=0; i<3; i++)
		{
			if(SQLITE_OK == sqlite3_exec( db , sql , 0 , 0 , &zErrMsg ))
			{
