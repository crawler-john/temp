...
int setlimitedpowerone(inverter_info *inverter, char power)//发送命令，设置单个逆变器限定功率
{
	...
	printhexmsg("Set maximun power single", sendbuff, 13);
...
int setlimitedpowerall(char power)	//发送广播命令，设置所有逆变器限定功率
{
	...
	printhexmsg("Set maximun power", sendbuff, 14);

	return 0;
}

int setstationarypowerone(inverter_info *inverter, char power)		//发送命令，设置单个逆变器固定功率
{
	unsigned char sendbuff[512] = {'\0'};
	unsigned short check = 0x00;
	int i=0;
	...
	zb_send_cmd(inverter, sendbuff, 13);
	sleep(1);
	printhexmsg("Set fixed power", sendbuff, 13);
...
int setstationarypowerall(char power)	//发送广播命令，设置所有逆变器固定功率
{
	...
	printhexmsg("Set fixed power", sendbuff, 14);
				...
				for(k=0; (k<MAXINVERTERCOUNT)&&(12==strlen(curinverter->inverterid)); k++, curinverter++)
				{
					if(!strncmp(azResult[j*ncolumn], curinverter->inverterid, 12))
					{
						...
						for(m=0; m<3; m++)
						{
							...
							if(1 == res)
							{
								limitedresult = (readpresetdata[7] << 16) / 7395;
								updatemaxpower(curinverter, limitedresult);
		...
		else if (!strncmp(command, "ALL", 3))
		{
			...
			for (i = 0;(i < MAXINVERTERCOUNT) && (12 == strlen(curinverter->inverterid));i++, curinverter++)
			{
				...
				else
				{
					for (j = 0; j < 3; j++)
					{
						setlimitedpowerone(curinverter, stationaryvalue);
						memset(readpresetdata, '\0', 20);
						res = zb_query_protect_parameter(curinverter,readpresetdata);
						if ((1 == res) && (stationaryvalue == readpresetdata[6]))
						{
	...
	if(!strcmp(id, "ALL"))
	{
		...
		for(i=0; (i<MAXINVERTERCOUNT)&&(12==strlen(curinverter->inverterid)); i++, curinverter++)
		{
			for(j=0; j<3; j++)
			{
				...
				if(1 == res)
				{
					limitedresult = (readpresetdata[7] << 16) / 7395;					//读取成功
					updatemaxpower(curinverter, limitedresult);
					//updatemaxflag(curinverter);
					save_max_power_result_one(curinverter, limitedresult);
					break;
...
//	process_fixed_power(firstinverter);
	read_max_power(firstinverter);			//ZK,暂时屏蔽
}
