...
int update_inverter_version(char *id, char version)					//更新逆变器的机型码和软件版本号
{
	...
	sprintf(sql, "UPDATE inverter_version SET version=%d WHERE id='%s' ", version, id);
...
int query_inverter_version(char *id)		//请求逆变器的机型码
{
	...
	{
		update_inverter_version(id, readbuff[22]);
		printdecmsg(id, readbuff[22]);
		return readbuff[22];
	}
	else
	{
...
int get_inverter_version_all(struct inverter_info_t *firstinverter)
{
	...
	fp = fopen("/temp/get_inverter_version.conf", "r");
	if(fp)
	{
		...
		if(strcmp("ALL", buff))
		{
			...
			fp = fopen("/temp/get_inverter_version.conf", "w");
			fclose(fp);
		}
