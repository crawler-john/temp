...
void add_functions()
{
	...
	pfun[A141] = set_encryption;				//重置防盗密码
	pfun[A142] = clear_encryption;				//擦除防盗密码
	pfun[A143] = response_inverter_encryption_time;//上报逆变器防盗时间
	pfun[A144] = set_encryption_time;			//设置防盗时间
...
//	debug_msg("hour:%d", timenow.tm_hour);
	...
	{
		//查询是否有flag=2的数据
		snprintf(sql, sizeof(sql), "SELECT COUNT(*) FROM inverter_status WHERE flag=2 AND date_time>'20150101000000'");
...
int exist_inverter_abnormal_status()
{
	...
	if(!open_db("/home/record.db", &db))
	{
		...
		snprintf(sql, sizeof(sql), "SELECT COUNT(*) FROM inverter_status WHERE flag=1 AND date_time>'20150101000000'");
		if(!get_data(db, sql, &azResult, &nrow, &ncolumn)){
			if(azResult[1] != NULL){
				if(atoi(azResult[1]) > 0){
					result = 1;
...
int response_inverter_abnormal_status()
{
	...
	if(!open_db("/home/record.db", &db))
	{
		...
		nrow = 0;
		memset(sql, 0, sizeof(sql));
		snprintf(sql, sizeof(sql), "SELECT item,result FROM inverter_status WHERE flag=1 AND date_time>'20150101000000'");
	...
	if(!open_db("/home/database.db", &db))
	{
		...
		while(item_num[num] != 0)
		{
			...
			else if(item==143)
			{
				...
				char ttm2[3]={'\0'};
				get_time(ttm,ttm2);
				sprintf(sendbuffer, "APS1300000A%03dAAA0%.12s0%04d00000000000000%.14sEND", item, ecuid, nrow,ttm);
			}
			else
			...
			if((item==147))
			{debug_msg("ddd1");
				if(recv_time(sockfd, recv_buffer, sizeof(recv_buffer), sockcfg.timeout) < 0){debug_msg("ddd5");
					close(sockfd);
					if(nrow)sqlite3_free_table(azResult);
					continue;
				}
				if(strncmp(recv_buffer,&sendbuffer[34],14))
					{debug_msg("ddd2");if(nrow)sqlite3_free_table(azResult);continue;}
				for(i=1; i<=nrow; i++)
				{debug_msg("ddd3");
					memset(sql, 0, sizeof(sql));
					snprintf(sql, sizeof(sql), "UPDATE inverter_process_result SET flag=0 WHERE result='%s'", azResult[i]);
					update_data(db, sql);debug_msg(sql);
				}
