...
int main(void)
{
	...
	{
		pid = getpid();
		printdecmsg("Starting phone_server", pid);
		execl("/home/applications/phone_server", NULL);
		exit(0);
	}
	...
	while(1)
	{
		...
		printmsg("phone_server has been stoped");			//如果运行到此处，说明main.exe已经中止，重新运行main.exe
		sleep(10);
		appid = vfork();
		if(appid == 0)
		{
