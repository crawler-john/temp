		...
		for(i=0; (i<MAXINVERTERCOUNT)&&(12==strlen(curinverter->id)); i++){
			if('1' == curinverter->dataflag){
				if(1 == curinverter->model){
					...
					sendbuff[j++] = curinverter->op*100%256;
					sendbuff[j++] = (int)(curinverter->gf*10)/256;
					sendbuff[j++] = (int)(curinverter->gf*10)%256;
					sendbuff[j++] = curinverter->it;
					sendbuff[j++] = curinverter->gv/256;
					sendbuff[j++] = curinverter->gv%256;
					sendbuff[j++] = curinverter->op*10.0/curinverter->gv;		//交流电流
