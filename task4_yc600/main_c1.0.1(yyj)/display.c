	...
	if(fp)
	{
		for(i=0; (i<MAXINVERTERCOUNT) && (12 == strlen(inverter->id)); i++)
		{
			...
			{				//ZK,YC250
				if(1 == inverter->dataflag)
				{
					fprintf(fp,"%s, %d, %.1f, %d, %d, %s\n", inverter->id, inverter->op, inverter->gf, inverter->gv, inverter->it, datatime);
				}
				else
				{
					fprintf(fp,"%s,  ,  ,  ,  , -\n", inverter->id);
			...
			else if((4 == inverter->model)||(3 == inverter->model)||(7 == inverter->model)) //7:YC600
			{		//ZK,YC500
				if(1 == inverter->dataflag)
				{
					fprintf(fp,"%s-A, %d, %.1f, %d, %d, %s\n", inverter->id, inverter->op, inverter->gf, inverter->gv, inverter->it, datatime);
					fprintf(fp,"%s-B, %d, %.1f, %d, %d, %s\n", inverter->id, inverter->opb, inverter->gf, inverter->gv, inverter->it, datatime);
				}
				else
				{
					fprintf(fp,"%s-A,  ,  ,  ,  , -\n", inverter->id);
			...
			{		//YC1000
				if( 1 == inverter->dataflag)
				{
					...
					fprintf(fp,"%s-2, %d, %.1f, B: %d, %d, %s\n", inverter->id, inverter->opb, inverter->gf, inverter->gvb, inverter->it, datatime);
					fprintf(fp,"%s-3, %d, %.1f, C: %d, %d, %s\n", inverter->id, inverter->opc, inverter->gf, inverter->gvc, inverter->it, datatime);
					fprintf(fp,"%s-4, %d, %.1f, -, %d, %s\n", inverter->id, inverter->opd, inverter->gf, inverter->it, datatime);
				}
				else
				{
					fprintf(fp,"%s-1,  ,  ,  ,  , -\n", inverter->id);
