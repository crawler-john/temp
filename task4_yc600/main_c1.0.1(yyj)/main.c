...
int main(int argc, char *argv[])
{
	...
	system("killall drm_connect.exe");
	fp = fopen("/etc/yuneng/area.conf", "r");
	if(fp)
	{
		fgets(version_saa, sizeof(version_saa), fp);
		if('\n' == version_saa[strlen(version_saa)-1])
			version_saa[strlen(version_saa)-1] = '\0';
		if(!strncmp(&version_saa[strlen(version_saa)-3], "SAA", 3))
			system("/home/applications/drm_connect.exe &");
		fclose(fp);
	}
	while(1){
