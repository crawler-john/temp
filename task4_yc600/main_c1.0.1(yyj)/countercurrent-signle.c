...
int get_reply_from_module(char *data)			//读取模块的返回帧
{
	int ret, size;
	fd_set rd;
...
//	printhexmsg("metergetdata",sendbuff,14);
	for(j=0; j<3; j++){              //发送3次
		...
		recvbytes=get_reply_from_module(data);
		...
		if(1 == metergetdata(meter_data,chip))
		{
			...
			if(0x02 == meter_data[6])  //通道2，数据存入文件
				...
				data[25]=energyC;
					...
					if(powerA > 32767)  //负数
					{
						data[17]=powerA-65535;
					}
					else
						data[17]=powerA;
					if(powerB > 32767)  //负数
					{
						data[18]=powerB-65535;
					}
					else
						data[18]=powerB;
					if(powerC > 32767)  //负数
					{
						data[19]=powerC-65535;
					}
					else
						data[19]=powerC;
				}
...
int meterset(char data[20][8])              //设置 返回1设置成功 返回0设置失败
{
	...
	for(j=0; j<3; j++){              //发送3次
		...
		recvbytes=get_reply_from_module(recvdata);
